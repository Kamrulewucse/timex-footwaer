    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <!-- /.box-header -->
                <!-- form start -->
                <form class="form-horizontal" method="POST" action="{{ route('branch_cash_add') }}">
                    @csrf
                            ...
                            <div class="col-sm-10">
                                <select class="form-control select2" name="branch">
                                    ...
                                    @foreach($branches as $branch)
                                        <option value="{{ $branch->id }}" {{ old('branch') == $branch->id ? 'selected' : '' }}>{{ $branch->companyBranch->name ?? '' }}</option>
                                    @endforeach
                                </select>
