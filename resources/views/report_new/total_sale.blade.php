                    ...
                    <div id="prinarea">
                        <div class="row">
                            <div class="col-md-12 text-center">
                                    ...
                                    <h5 style="margin-bottom: 0px;margin-top: 0px;padding: 0">Sale @if(request()->get('start')), Date:
                        ...
                        <table id="table" class="table table-bordered">
                            ...
                            @foreach($dateWiseSales as $dateWiseSale)
                                <tr>
                                    <td class="text-center">Date</td>
                                    <td colspan="6">{{ date('d-m-Y',strtotime($dateWiseSale->date)) }}</td>
                                </tr>
                                @php
                                   $orders = $dateWiseSale->sale($dateWiseSale->date);
                                @endphp
                                @foreach($orders as $order)
                                    <tr>
                                        <td class="text-center">{{$order->order_no}}</td>
                                        <td class="text-center">{{$order->customer->id_no??''}}</td>
                                ...
                                <tr>
                                    ...
                                    <td colspan="2">{{$dateWiseSale->totalDaySale}}</td>
                                    <td>{{$dateWiseSale->totalPaid}}</td>
                                    <td class="text-right">{{$dateWiseSale->totalDiscount}}</td>
                                    <td class="text-right">Tk  {{number_format($dateWiseSale->totalDue,2)}}</td>
                                    <td class="text-right">Tk  {{number_format($dateWiseSale->grandTotal,2)}}</td>
                                </tr>
                                <?php
