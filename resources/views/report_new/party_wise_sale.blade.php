    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('report_party_wise_sale') }}">
                        <div class="row">
                            <div class="col-md-3">
                                ...
                                <div class="form-group row">
                                    ...
                                    <select class="form-control select2" name="customer" required>
                                        ...
                                        @foreach($customers as $customer)
                                            <option value="{{ $customer->id }}" {{ request()->get('customer') == $customer->id ? 'selected' : '' }}>{{ $customer->name }} - @if($customer->type==1)Retail Customer @else WholeSale Customer @endif</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                    ...
                    <div id="prinarea">
                        <div class="row">
                            <div class="col-md-12 text-center">
                                    ...
                                    <h5 style="margin-bottom: 0px;margin-top: 0px;padding: 0">Party Wise Sale @if(request()->get('start')), Date:
                        ...
                        <table id="table" class="table table-bordered">
                            ...
                            @foreach($orderProducts as $orderProduct)
                                <tr>
                                    <td class="text-center">{{$orderProduct->productItem->id??''}}</td>
                                    <td>{{$orderProduct->productItem->name??''}}</td>
                                    <td class="text-center">Shoe</td>
                                    <td class="text-right">{{$orderProduct->quantity}}</td>
                                    <td class="text-right">Tk  {{number_format($orderProduct->total/$orderProduct->quantity,2)}}</td>
                                    <td class="text-right">Tk  {{number_format($orderProduct->total,2)}}</td>
                                </tr>
