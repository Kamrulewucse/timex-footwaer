...
@section('content')
    <form method="POST" enctype="multipart/form-data" action="{{ route('retail_sale_order_create') }}" id="sale_form">
        @csrf
    ...
    <script>
        $(function () {
            ...
            $('#newCustomerForm').submit(function (e) {
                ...
                $.ajax({
                    method: "Post",
                    url: "{{ route('add_ajax_customer',['type'=>1]) }}",
                    data: formData,
                    processData: false,
                    contentType: false,
            ...
            $('#serial_search').on('input',function () {
                ...
                if (term != '') {
                    $.ajax({
                        ...
                        if (response.success) {
                            $.each(response.productsIds, function (index,row) {
                                let html = '<div class="col-md-6 col-12 suggestion_item">'+
                                    '<div class="product_suggestion" style="cursor: pointer;" data-serial="'+row.serial+'">'+
                                    '<h6>'+row.product_item.name+" - "+row.product_category.name+'</h6>'+
                                    '</div>'+
                                    '</div>';
        ...
        function initSelect2() {
            ...
            $('.customer').select2({
                ajax: {
                    url: "{{ route('get_customer_json',['type'=>1]) }}",
                    type: "get",
                    dataType: 'json',
                    delay: 250,
